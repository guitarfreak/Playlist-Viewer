...
void sendHttpRequest(void* data) {
    ...
    if(findStart) strCpy(tempLink, tempLink+findStart+3);

    int hasContent = strFind(tempLink, '/');
    if(hasContent) {
        copySubstring(hostAndPort, tempLink, 0, strFind(tempLink, '/')-2);
        copySubstring(hirarchy, tempLink, strFind(tempLink, '/')-1, strLen(tempLink));
    } else {
    ...
    if(connect(webSocket, (SOCKADDR*)&sockAddr, sizeof(sockAddr)) != 0) {
        printf("Could not connect, ErrorCode: %i.\n", WSAGetLastError());
